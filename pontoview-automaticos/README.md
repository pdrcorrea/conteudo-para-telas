...
- `sustentabilidade/` - qualidade do ar configurável por coordenadas e indicadores ambientais do Brasil.

## Fontes gratuitas

- BrasilAPI: feriados e taxas oficiais.
- AwesomeAPI Economia: cotações de moedas e Bitcoin.
- Wikimedia / Wikipédia: conteúdo enciclopédico e imagens quando disponíveis.
- Open-Meteo Air Quality: qualidade do ar por coordenadas.
- World Bank Indicators API: séries ambientais do Brasil.

As fontes podem alterar formato ou disponibilidade sem aviso. Por isso, todos os painéis possuem fallback visual e tratamento de ausência de dados.
...
Os painéis funcionam também tentando as fontes públicas diretamente. Para usar o Worker, acrescente o parâmetro `api` à URL do painel.
...
O painel aceita localização por query string:
...
O projeto usa o logo PontoView já presente em `assets/logo/pontoview-logo-white.png` no repositório principal. Os painéis mantêm a mesma linguagem visual dos demais módulos PontoView, com fundo azul, tipografia de alta leitura para TV, safe area, animações discretas e adaptação a 16:9 e 9:16.
